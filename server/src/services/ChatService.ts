...
export class ChatService {
  ...
  private handleJoin(socket: Socket, data: JoinRequest) {
    ...
    const displayName = sanitizeText(data.displayName.trim());
    ...
    // 他のユーザーに参加通知
    socket.broadcast.emit('userJoined', { username: displayName });

    // 全員にユーザー数を通知
    this.io.emit('userCount', { count: this.users.size });

    console.log(`${displayName} (${username}) が参加しました (合計: ${this.users.size}人)`);
  }
  ...
  private handleSendMessage(socket: Socket, data: SendMessageRequest) {
    ...
    console.log(`${user.displayName} (${user.username}): ${content}`);
  }
  ...
  private handleDisconnect(socket: Socket) {
    ...
    if (user) {
      ...
      // 他のユーザーに退室通知
      socket.broadcast.emit('userLeft', { username: user.displayName });

      // 全員にユーザー数を通知
      this.io.emit('userCount', { count: this.users.size });

      console.log(`${user.displayName} (${user.username}) が退室しました (残り: ${this.users.size}人)`);
    }
  }
